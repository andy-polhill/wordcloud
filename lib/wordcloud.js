...
class WordCloud {
  ...
  static get MAX_TEXT_SIZE() { return 10 }; //vw
  static get MIN_TEXT_SIZE() { return 0.2 }; //vw
  static get SPIRAL_STEP() { return 1.5 }; //lower is more dense but less performant
  ...
  selectTopic(evt) {
    if(evt.target.tagName === 'text') {
      document.dispatchEvent(new CustomEvent(WordCloud.TOPIC_SELECT_EVENT, {
  ...
  static createPosition(angle, count) {
    ...
    var y = 50;
    if(count > 0) {
      x += ((count * WordCloud.SPIRAL_STEP)) * Math.cos(angle);
      y += ((count * WordCloud.SPIRAL_STEP)) * Math.sin(angle);
    }
